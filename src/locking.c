...
int LIMD_lock_file(const char* filename, LIMD_lock_info_t* lockinfo)
{
    if (!lockinfo) {
        return -1;
    }
    ...
    if (!lockinfo->fp) {
        LIMD_log_message(LIMD_DEBUG, "locking", "ERROR: could not open or create lockfile '%s'\n", filename);
        return -1;
    }
    ...
    if (fcntl(fileno(lockinfo->fp), F_SETLKW, &lockinfo->ldata) < 0) {
        LIMD_log_message(LIMD_DEBUG,"locking", "ERROR: can't lock file, error %d\n", errno);
        fclose(lockinfo->fp);
        lockinfo->fp = NULL;
        return -1;
    }
...
int LIMD_unlock_file(LIMD_lock_info_t* lockinfo)
{
    if (!lockinfo) {
        return -1;
    }
    ...
    if (fcntl(fileno(lockinfo->fp), F_SETLK, &lockinfo->ldata) < 0) {
        LIMD_log_message(LIMD_DEBUG,"locking","ERROR: can't unlock file, error %d\n", errno);
        fclose(lockinfo->fp);
        lockinfo->fp = NULL;
        return -1;
    }
